...
class MapProvider extends ChangeNotifier {
  ...
  void setMapPins(
    ...
    List<TublesModel> destinationList,
  ) async {
    ...
    for (int i = 0; i < destinationList.length; i++) {
      ...
      // BARU: Hitung jarak statis dan simpan ke model
      if (sourceLocation != null) {
        String calculatedDistance = await calculateDistance(
          sourceLocation,
          data.location!,
        );
        data.distanceString = calculatedDistance;
      }
  ...
  void dialogCheckin(BuildContext context) {
    DialogUtils.showDialogChoose(
      context,
      "Akhiri Navigasi ⛔",
      "Sudah sampai di bengkel tujuan?",
      () {
        //On Yes
        Navigator.pop(context);
        stopNavigate();
